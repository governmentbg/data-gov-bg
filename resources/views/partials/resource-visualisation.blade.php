                    ...
                    @if (!in_array($format,\App\Resource::FORMAT_LIMITS[App\Resource::getFormats()[$versionFormat]]))
                        <div class="form-group row">
                            <div class="col-sm-12" style="position: relative;padding-right:100px;">
                                ...
                                <input type="text" id="link_{{ strtolower($format) }}" class="form-control" value="{{ url('/resource/download') }}?_token={{ csrf_token() }}&resource={{ $resource->id }}&version={{ $versionView }}&name={{ urlencode($resource->name) }}&format={{ $format }}" readonly>
